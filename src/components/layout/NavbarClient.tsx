...
function getProfileInitials(user: NavbarInitialUser | null) {
...
/**
 * Client-side shell that owns presentation state (scroll, menu toggle) and
 * listens for `auth-state-changed` to ask the router to refresh — that re-runs
 * the parent server component which re-reads the cookie and re-renders this
 * client with fresh `initialAuthenticated` / `initialUser` props.
 *
 * No client-side `/api/auth/session` fetch, no focus/visibility polling. The
 * server cookie read is the single source of truth per render.
 */
export default function NavbarClient({ initialAuthenticated, initialUser }: NavbarClientProps) {
  ...
  const ctaHref = initialAuthenticated ? '/contact' : '/register'
  const ctaLabel = initialAuthenticated ? 'Book Now' : 'Register'
  const mobileNavigationLinks = initialAuthenticated
    ? [...navLinks, { href: '/me', label: 'My Space' }]
    : navLinks
  const profileInitials = getProfileInitials(initialUser)
    ...
    >
      ...
      >
        ...
        <Link href="/" className="flex items-center gap-2.5">
          ...
          />
          <span
            className="hidden min-w-0 flex-col justify-center leading-none sm:flex"
            aria-label={`${siteConfig.shortName} with Dr. Didi`}
          >
            <span
              className="font-heading text-[15px] font-bold uppercase tracking-normal"
              style={{ color: 'var(--color-primary)' }}
            >
              DOWNBELOW
            </span>
            <span
              className="mt-0.5 font-body text-[10px] font-bold leading-none"
